...
function getRoofSlopeConversions(){
  const ratio=getRoofSlopeRatio();
  const sun=ratio*10;
  const angle=Math.atan(ratio)*180/Math.PI;
  const percent=ratio*100;
  return {ratio,sun,angle,percent};
}
...
function updateRoofCoefficientUI(){
  ...
  if(note){
    if(type===1){
      note.textContent="大波スレート：初期設定で「屋根材から選択」とし、標準係数 1.140 を自動表示します。";
    }else if(type===2){
...
function sealLengthForSource(src){
  if(src==="tank") return Number(state.tankSeal)||0;
  const rowId={roof:"roofJointRows",flat:"flatJointRows",vessel:"vesselJointRows"}[src];
  if(rowId && $(rowId)) return jointTotal(rowId);
  return Number({roof:state.roofJoint,flat:state.flatJoint,vessel:state.vesselJoint}[src])||0;
}
